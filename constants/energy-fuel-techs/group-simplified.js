import * as FT from './group-detailed.js'

export const GROUP_LABEL = 'Simplified'
export const GROUP_NAME = 'group.simplified'

export const NAME_PUMPS = `pumps`
export const NAME_BATTERY_CHARGING = `battery_charging`
export const NAME_EXPORTS = `exports`
export const NAME_IMPORTS = `imports`
export const NAME_BIOENERGY = `bioenergy`
export const NAME_DISTILLATE = `distillate`
export const NAME_BATTERY_DISCHARGING = `battery_discharging`
export const NAME_BATTERY = `battery`
export const NAME_HYDRO = `hydro`
export const NAME_WIND = `wind`
export const NAME_COAL = `coal`
export const NAME_GAS = `gas`
export const NAME_SOLAR = `solar`

export const PUMPS = `${GROUP_NAME}.${NAME_PUMPS}`
export const BATTERY_CHARGING = `${GROUP_NAME}.${NAME_BATTERY_CHARGING}`
export const EXPORTS = `${GROUP_NAME}.${NAME_EXPORTS}`
export const IMPORTS = `${GROUP_NAME}.${NAME_IMPORTS}`

export const BIOENERGY = `${GROUP_NAME}.${NAME_BIOENERGY}`
export const DISTILLATE = `${GROUP_NAME}.${NAME_DISTILLATE}`
export const BATTERY_DISCHARGING = `${GROUP_NAME}.${NAME_BATTERY_DISCHARGING}`
export const BATTERY = `${GROUP_NAME}.${NAME_BATTERY}`
export const HYDRO = `${GROUP_NAME}.${NAME_HYDRO}`
export const WIND = `${GROUP_NAME}.${NAME_WIND}`
export const COAL = `${GROUP_NAME}.${NAME_COAL}`
export const GAS = `${GROUP_NAME}.${NAME_GAS}`
export const SOLAR = `${GROUP_NAME}.${NAME_SOLAR}`

export const NAME_FUEL_TECH_GROUP = {}
NAME_FUEL_TECH_GROUP[PUMPS] = NAME_PUMPS
NAME_FUEL_TECH_GROUP[BATTERY_CHARGING] = NAME_BATTERY_CHARGING
NAME_FUEL_TECH_GROUP[EXPORTS] = NAME_EXPORTS
NAME_FUEL_TECH_GROUP[IMPORTS] = NAME_IMPORTS
NAME_FUEL_TECH_GROUP[BIOENERGY] = NAME_BIOENERGY
NAME_FUEL_TECH_GROUP[DISTILLATE] = NAME_DISTILLATE
NAME_FUEL_TECH_GROUP[BATTERY_DISCHARGING] = NAME_BATTERY_DISCHARGING
NAME_FUEL_TECH_GROUP[BATTERY] = NAME_BATTERY
NAME_FUEL_TECH_GROUP[HYDRO] = NAME_HYDRO
NAME_FUEL_TECH_GROUP[WIND] = NAME_WIND
NAME_FUEL_TECH_GROUP[COAL] = NAME_COAL
NAME_FUEL_TECH_GROUP[GAS] = NAME_GAS
NAME_FUEL_TECH_GROUP[SOLAR] = NAME_SOLAR

export const FUEL_TECH_GROUP = {}
FUEL_TECH_GROUP[PUMPS] = [FT.PUMPS]
FUEL_TECH_GROUP[BATTERY_CHARGING] = [FT.BATTERY_CHARGING]
FUEL_TECH_GROUP[EXPORTS] = [FT.EXPORTS]
FUEL_TECH_GROUP[IMPORTS] = [FT.IMPORTS]
FUEL_TECH_GROUP[BIOENERGY] = [
  FT.BIOENERGY_BIOMASS,
  FT.BIOENERGY_BIOGAS
]
FUEL_TECH_GROUP[DISTILLATE] = [FT.DISTILLATE]
FUEL_TECH_GROUP[BATTERY_DISCHARGING] = [FT.BATTERY_DISCHARGING]
FUEL_TECH_GROUP[BATTERY] = [FT.BATTERY]
FUEL_TECH_GROUP[HYDRO] = [FT.HYDRO]
FUEL_TECH_GROUP[WIND] = [FT.WIND]
FUEL_TECH_GROUP[COAL] = [
  FT.COAL_BROWN,
  FT.COAL_BLACK
]
FUEL_TECH_GROUP[GAS] = [
  FT.GAS_STEAM,
  FT.GAS_CCGT,
  FT.GAS_OCGT,
  FT.GAS_RECIP,
  FT.GAS_LFG,
  FT.GAS_WCMG
]
FUEL_TECH_GROUP[SOLAR] = [
  FT.SOLAR_UTILITY,
  FT.SOLAR_ROOFTOP
]

// Fuel tech group order
export const FUEL_TECH_ORDER = [
  BATTERY_CHARGING,
  PUMPS,
  EXPORTS,
  IMPORTS,
  COAL,
  BIOENERGY,
  DISTILLATE,
  GAS,
  BATTERY_DISCHARGING,
  BATTERY,
  HYDRO,
  WIND,
  SOLAR
]

// Fuel tech group colour
export const FUEL_TECH_GROUP_COLOUR = {}
FUEL_TECH_GROUP_COLOUR[PUMPS] = '#88AFD0'
FUEL_TECH_GROUP_COLOUR[BATTERY_CHARGING] = '#577CFF'
FUEL_TECH_GROUP_COLOUR[EXPORTS] = '#927BAD'
FUEL_TECH_GROUP_COLOUR[IMPORTS] = '#521986'
FUEL_TECH_GROUP_COLOUR[COAL] = '#25170C'
FUEL_TECH_GROUP_COLOUR[BIOENERGY] = '#1D7A7A'
FUEL_TECH_GROUP_COLOUR[DISTILLATE] = '#E15C34'
FUEL_TECH_GROUP_COLOUR[GAS] = '#E87809'
FUEL_TECH_GROUP_COLOUR[BATTERY_DISCHARGING] = '#3245c9'
FUEL_TECH_GROUP_COLOUR[BATTERY] = '#3245c9'
FUEL_TECH_GROUP_COLOUR[HYDRO] = '#5EA0C0'
FUEL_TECH_GROUP_COLOUR[WIND] = '#2C7629'
FUEL_TECH_GROUP_COLOUR[SOLAR] = '#FED500'

// Fuel tech type
const LOAD = 'load'
const SOURCE = 'source'
export const FUEL_TECH_CATEGORY = {}
FUEL_TECH_CATEGORY[PUMPS] = LOAD
FUEL_TECH_CATEGORY[BATTERY_CHARGING] = LOAD
FUEL_TECH_CATEGORY[EXPORTS] = LOAD
FUEL_TECH_CATEGORY[IMPORTS] = SOURCE
FUEL_TECH_CATEGORY[COAL] = SOURCE
FUEL_TECH_CATEGORY[BIOENERGY] = SOURCE
FUEL_TECH_CATEGORY[DISTILLATE] = SOURCE
FUEL_TECH_CATEGORY[GAS] = SOURCE
FUEL_TECH_CATEGORY[BATTERY_DISCHARGING] = SOURCE
FUEL_TECH_CATEGORY[BATTERY] = SOURCE
FUEL_TECH_CATEGORY[HYDRO] = SOURCE
FUEL_TECH_CATEGORY[WIND] = SOURCE
FUEL_TECH_CATEGORY[SOLAR] = SOURCE

// Fuel tech label
export const FUEL_TECH_LABEL = {}
FUEL_TECH_LABEL[PUMPS] = 'Pumps'
FUEL_TECH_LABEL[BATTERY_CHARGING] = 'Battery (Charging)'
FUEL_TECH_LABEL[EXPORTS] = 'Exports'
FUEL_TECH_LABEL[IMPORTS] = 'Imports'
FUEL_TECH_LABEL[COAL] = 'Coal'
FUEL_TECH_LABEL[BIOENERGY] = 'Bioenergy'
FUEL_TECH_LABEL[DISTILLATE] = 'Distillate'
FUEL_TECH_LABEL[GAS] = 'Gas'
FUEL_TECH_LABEL[BATTERY_DISCHARGING] = 'Battery (Discharging)'
FUEL_TECH_LABEL[BATTERY] = 'Battery'
FUEL_TECH_LABEL[HYDRO] = 'Hydro'
FUEL_TECH_LABEL[WIND] = 'Wind'
FUEL_TECH_LABEL[SOLAR] = 'Solar'
